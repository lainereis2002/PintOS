...
pintos_init (void)
{
  ...
  if (*argv != NULL) {
    ...
    while (true) {
      ...
      while (true) {
        uint8_t key = input_getc(); //get the input character
        if ((char)key == 13) { //if enter is pressed make the last index of the array terminating character
            input[index] = '\0';
            break;
        }
        else{
          putchar(key); //print character to the console
        }
        input[index] = key;
        index++;
      }

      //check the input and execute the relevant command
      if(strcmp(input,"whoami")==0){
		    printf("\nYour name: Gislaine Maria Valeska Brandão Reis\n");
      }
      else if(strcmp(input,"exit")==0){
        printf("\nExiting from the interactive shell...Good Bye!\n");
        break;
      }
      else{
        printf("\nInvalid command!");
